...
int read_overhead(FILE* istream, char extension[MAX_EXT_LENGTH], size_t* file_size, size_t freq_count[ASCII_ALP_SIZE]) {
	...
	if (extension == NULL) {
		if (fseek(istream, MAX_EXT_LENGTH, SEEK_CUR)) {
			perror("Ошибка установления каретки в необходимое положение");
			return -1;
		}
	} else {
		if (fread(extension, sizeof(*extension), MAX_EXT_LENGTH, istream) != MAX_EXT_LENGTH) {
			perror("Ошибка чтения служебной информации файла");
			return 1;
		}
	}
...
/*
	Что надо переделать? [ПЕРЕДЕЛАНО]
...
static int huffman_decompress(
	...
	// чистим прошлые ошибки для корректных проверок
	clearerr(istream);
	clearerr(ostream);
	...
	if (read_count < READ_BUFFER_SIZE && ferror(istream)) {
		perror("Ошибка чтения файла в стековый буфер");
		return 0;
	}
		...
		*write_ptr++ = cur_node->sym;
		...
		if (write_ptr >= &write_buf[WRITE_BUFFER_SIZE]) {
			...
			if (written < WRITE_BUFFER_SIZE) {
				perror("Ошибка записи данных на диск");
				return 0;
			}
...
int decompress_file_v1(
	...
	int success = huffman_decompress(istream, ostream, root_tree, file_size);
